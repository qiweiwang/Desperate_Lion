...
void MC_encoder(unsigned char * beforeAdd, unsigned char * afterAdd, int length)
{
	int i;
        memset(afterAdd, 0x00, 2*length);
        for (i=0; i<length; i++)
	{
		if (beforeAdd[i]==0x40)
		{
			...
			{      
				// printf("packet %d sent!, batch 0 empty\n", counterS);
                                ...
                                // nanosleep(&delayTime, NULL);
                                 usleep(100);
	       			 while(readTotal<((HEADERSIZE+DATASIZE)*2))
	      			 {      
	       				readCount=read(sockfd, (unsigned char *)(sharedMem_int+10)+readTotal, (HEADERSIZE+DATASIZE)*2-readTotal);
...
/*               for(; counterJ<64; counterJ++)
		{
			printf("0x%x\n", *((unsigned char *)(sharedMem_int+10)+counterJ));
                        if(counterJ%2 == 1)
			printf("\n");
		}
			  ...
			  //      printf("packet %d sent! batch 1 empty\n", counterS);
				...
				usleep(100);
              			 while(readTotal<((HEADERSIZE+DATASIZE)*2))
	      			 {
	   			    	...
	   			    	readCount=read(sockfd, (unsigned char *)(sharedMem_int+18)+(HEADERSIZE+DATASIZE)*2+readTotal, (HEADERSIZE+DATASIZE)*2-readTotal);
	     				if (readCount>=0)
					readTotal+=readCount;
